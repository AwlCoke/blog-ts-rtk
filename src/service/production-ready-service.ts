...
export const getArticlesList: (a?: number) => Promise<{ articles: ArticleModel[], articlesCount: number }> = async (offset = 0) => {
    let response;
...
export const updateArticle: (a: string, b: CreateArticleType) => Promise<any> = (slug: string, article: CreateArticleType) => {
...
export const login: (a: UserLoginType) => Promise<any> = (user: UserLoginType) => {
