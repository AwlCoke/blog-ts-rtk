...
import { FormDataArticle } from '../../../types/forms';
...
const ArticleForm: FC<Props> = ({ mode, slug, getArticles }: Props) => {
  ...
  const articleFormContent = ArticleFormProps.map((field: any) => {
    ...
    return (
      <UserForm key={`${field.name}_${props.id}`} {...invalidProps} ref={register(rules)} style={{ width: '100%' }} />
    );
  });
    ...
    return (
      <li key={tag} className={styles.tag}>
        ...
        <button type="button" onClick={() => removeTag(tag)}>
          <DeleteFilled style={{ color: '#F5222D', marginLeft: 5 }} />
        </button>
      </li>
    );
  });
  ...
  const tagFormContent = (
    <label className={styles.tagBlock}>
      ...
      <ul style={{ paddingLeft: 0 }}>
        ...
        {/* eslint-disable-next-line jsx-a11y/control-has-associated-label */}
        <button type="button" onClick={addTag} className={addBtn}>
          Add Tag
        </button>
      </ul>
    </label>
